...
try: 
    ...
    db_settlement_data= upload_conn.read(worksheet="settlement_upload")
    
    db_sales_data=upload_conn.read(worksheet="sales_upload")
    
    db_master_data=upload_conn.read(worksheet="master_upload")
    
    db_latlong=upload_conn.read(worksheet="latlong")
   ...
   with col2 :
    ...
    with subcol2 :
        if st.button('Upload',key="sales_btn"):
            ...
            db_sales=pd.concat([db_sales,db_sales_data],ignore_index=True,sort=False)  
