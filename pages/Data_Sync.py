...
with st.container(border=True) :
    ...
    if st.button('Sync Now',key="sync_btn"):
        ...
        db_settlement_upload=upload_conn.read(worksheet="settlement_upload")
        ...
        if len(db_settlement_upload)>0:
            ...
            master_conn.update(worksheet="settlement",data=db_settlement_all)     
        ...
        db_sales_upload=upload_conn.read(worksheet="sales_upload")
        ...
        if len(db_sales_upload)>0:
            ...
            upload_conn.clear(worksheet="sales_upload")
            master_conn.update(worksheet="sales",data=db_sales_all) 
        ...
        db_master_upload=upload_conn.read(worksheet="master_upload")
        ...
        if len(db_master_upload)>0:
            ...
            upload_conn.clear(worksheet="master_upload")
            master_conn.update(worksheet="master",data=db_master_all)
